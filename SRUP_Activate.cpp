...
bool SRUP_MSG_ACTIVATE::Serialize(bool preSign)
{
...
size_t SRUP_MSG_ACTIVATE::SerializedLength()
{
    if (!m_is_serialized)
        Serialize(false);
...
unsigned char *SRUP_MSG_ACTIVATE::Serialized()
{
    if (Serialize())
        return m_serialized;
    else
...
bool SRUP_MSG_ACTIVATE::DeSerialize(const unsigned char* serial_data)
{
    unsigned short x;
    unsigned int p=0;
...
bool SRUP_MSG_ACTIVATE::DataCheck()
{
    if ((m_token == nullptr) && (m_sequence_ID == nullptr))
        return false;
    else
        return true;
}
