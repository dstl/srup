...
SRUP_MSG_SYNDICATED_ID_REQ::SRUP_MSG_SYNDICATED_ID_REQ ()
{
    m_msgtype[0] = SRUP::SRUP_MESSAGE_TYPE_SYNDICATED_ID_REQUEST;
    m_target_ID = nullptr;
}
...
const uint64_t *SRUP_MSG_SYNDICATED_ID_REQ::targetID ()
{
    return m_target_ID;
}
...
bool SRUP_MSG_SYNDICATED_ID_REQ::DeSerialize (const uint8_t *serial_data)
{
    uint16_t x;
    uint32_t p=0;
    uint8_t bytes[2];
...
bool SRUP_MSG_SYNDICATED_ID_REQ::Serialize (bool preSign)
{
    ...
    if (preSign)
    {
        serial_len = m_token_len + len_target;
        var_length_field_count--;
    }
    else
        serial_len = m_sig_len + m_token_len + len_target;
...
bool SRUP_MSG_SYNDICATED_ID_REQ::DataCheck ()
{
    if ((m_token != nullptr) && (m_sequence_ID != nullptr) && (m_sender_ID != nullptr) && (m_target_ID != nullptr))
        return true;
    else
        return false;
}
