...
# The main test script for pySRUPLib's SRUP_ID_Request() class...


def test_ID_request_type():
    x = pySRUPLib.SRUP_ID_Request()
    assert x.msg_type == pySRUPLib.__id_request_message_type()


def test_ID_request_seqid():
...
def test_ID_request_serializer():
    x = pySRUPLib.SRUP_ID_Request()
    x.token = "TOKEN12345"
    x.sequence_id = 0x1234567890ABCDEF
    x.sender_id = 0x5F5F5F5F5F5F5F5F
...
def test_ID_request_deserializer():
    token = "TOKEN12345"
    seq_id = 0x1234567890ABCDEF
    send_id = 0x5F5F5F5F5F5F5F5F

    x = pySRUPLib.SRUP_ID_Request()
    y = pySRUPLib.SRUP_ID_Request()
