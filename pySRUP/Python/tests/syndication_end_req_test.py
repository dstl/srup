...
# The main test script for pySRUPLib's SRUP_Syndicated_End_Request() class...


def test_syndicated_end_req_type():
    x = pySRUPLib.SRUP_Syndicated_End_Request()
    assert x.msg_type == pySRUPLib.__syndicated_end_request_message_type()


def test_syndicated_end_req_seqid():
...
def test_syndicated_end_req_sender():
...
def test_syndicated_end_req_token():
    x = pySRUPLib.SRUP_Syndicated_End_Request()
...
def test_syndicated_end_req_signing():
    blank = ""
...
def test_syndicated_end_req_serializer():
    x = pySRUPLib.SRUP_Syndicated_End_Request()
    x.token = "TOKEN12345"
    x.sequence_id = 0x1234567890ABCDEF
    x.sender_id = 0x5F5F5F5F5F5F5F5F
...
def test_syndicated_end_req_deserializer():
    token = "TOKEN12345"
    seq_id = 0x1234567890ABCDEF
    send_id = 0x5F5F5F5F5F5F5F5F

    x = pySRUPLib.SRUP_Syndicated_End_Request()
    y = pySRUPLib.SRUP_Syndicated_End_Request()
...
def test_syndicated_end_req_generic_deserializer():
    token = "TOKEN12345"
    seq_id = 0x1234567890ABCDEF
    send_id = 0x5F5F5F5F5F5F5F5F
    ...
    assert i.msg_type == pySRUPLib.__syndicated_end_request_message_type()
