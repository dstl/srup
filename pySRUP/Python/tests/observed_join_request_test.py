...
# The main test script for pySRUPLib's SRUP_Observed_Join_Request() class...


def test_observed_join_request_type():
    x = pySRUPLib.SRUP_Observed_Join_Request()
    assert x.msg_type == pySRUPLib.__observed_join_request_message_type()


def test_observed_join_request_seqid():
...
def test_observed_join_request_sender():
...
def test_observed_join_request_token():
    x = pySRUPLib.SRUP_Observed_Join_Request()
...
def test_observed_join_request_signing():
    blank = ""
    ...
    x.observer_id = 0x8282828282828282
...
def test_observed_join_request_serializer():
    x = pySRUPLib.SRUP_Observed_Join_Request()
    x.token = "TOKEN12345"
    x.sequence_id = 0x1234567890ABCDEF
    x.sender_id = 0x5F5F5F5F5F5F5F5F
    x.observer_id = 0x5353535353535353
    assert x.sign(keyfile) is True
    z = x.serialize()


def test_observed_join_request_deserializer():
    token = "TOKEN12345"
    seq_id = 0x1234567890ABCDEF
    send_id = 0x5F5F5F5F5F5F5F5F
    observer_id = 0x8383838383838383

    x = pySRUPLib.SRUP_Observed_Join_Request()
    y = pySRUPLib.SRUP_Observed_Join_Request()

    x.token = token
    x.sequence_id = seq_id
    x.sender_id = send_id
    x.observer_id = observer_id
    ...
    assert y.observer_id == observer_id


def test_observed_join_request_generic_deserializer():
    token = "TOKEN12345"
    seq_id = 0x1234567890ABCDEF
    send_id = 0x5F5F5F5F5F5F5F5F
    observer_id = 0x8383838383838383
    ...
    assert i.msg_type == pySRUPLib.__observed_join_request_message_type()
