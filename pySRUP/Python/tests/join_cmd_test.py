...
# The main test script for pySRUPLib's SRUP_Join_Command() class...


def test_join_command_type():
    x = pySRUPLib.SRUP_Join_Command()
    assert x.msg_type == pySRUPLib.__join_command_message_type()


def test_join_command_seqid():
...
def test_join_command_signing():
    ...
    x.device_id = 0x8282828282828282
...
def test_join_command_serializer():
    x = pySRUPLib.SRUP_Join_Command()
    x.token = "TOKEN12345"
    x.sequence_id = 0x1234567890ABCDEF
    x.sender_id = 0x5F5F5F5F5F5F5F5F
    x.device_id = 0x5353535353535353
    assert x.sign(keyfile) is True
    z = x.serialize()


def test_join_command_deserializer():
    token = "TOKEN12345"
