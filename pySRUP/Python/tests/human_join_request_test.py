...
# The main test script for pySRUPLib's SRUP_Human_Join_Request() class...


def test_Human_Join_Request_type():
    x = pySRUPLib.SRUP_Human_Join_Request()
    assert x.msg_type == pySRUPLib.__human_join_request_message_type()


def test_Human_Join_request_seqid():
...
def test_Humna_Join_request_sender():
...
def test_Humna_Join_request_token():
    x = pySRUPLib.SRUP_Human_Join_Request()
...
def test_Human_Join_request_signing():
    blank = ""
...
def test_Human_Join_request_serializer():
    x = pySRUPLib.SRUP_Human_Join_Request()
    x.token = "TOKEN12345"
    x.sequence_id = 0x1234567890ABCDEF
    x.sender_id = 0x5F5F5F5F5F5F5F5F
...
def test_Human_Join_request_deserializer():
    token = "TOKEN12345"
    seq_id = 0x1234567890ABCDEF
    send_id = 0x5F5F5F5F5F5F5F5F

    x = pySRUPLib.SRUP_Human_Join_Request()
    y = pySRUPLib.SRUP_Human_Join_Request()
...
def test_Human_Join_request_generic_deserializer():
    token = "TOKEN12345"
    seq_id = 0x1234567890ABCDEF
    send_id = 0x5F5F5F5F5F5F5F5F
    ...
    assert i.msg_type == pySRUPLib.__human_join_request_message_type()
