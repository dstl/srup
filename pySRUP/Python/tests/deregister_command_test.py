...
# The main test script for pySRUPLib's SRUP_Deregister_Command() class...


def test_deregister_command_type():
    x = pySRUPLib.SRUP_Deregister_Command()
    assert x.msg_type == pySRUPLib.__deregister_command_message_type()


def test_deregister_command_request_seqid():
...
def test_deregister_command_sender():
...
def test_deregister_command_token():
    x = pySRUPLib.SRUP_Deregister_Command()
...
def test_deregister_command_signing():
    blank = ""
...
def test_deregister_command_serializer():
    x = pySRUPLib.SRUP_Deregister_Command()
    x.token = "TOKEN12345"
    x.sequence_id = 0x1234567890ABCDEF
    x.sender_id = 0x5F5F5F5F5F5F5F5F
...
def test_deregister_command_deserializer():
    token = "TOKEN12345"
    seq_id = 0x1234567890ABCDEF
    send_id = 0x5F5F5F5F5F5F5F5F

    x = pySRUPLib.SRUP_Deregister_Command()
    y = pySRUPLib.SRUP_Deregister_Command()
...
def test_deregister_command_generic_deserializer():
    token = "TOKEN12345"
    seq_id = 0x1234567890ABCDEF
    send_id = 0x5F5F5F5F5F5F5F5F
    ...
    assert i.msg_type == pySRUPLib.__deregister_command_message_type()
