...
SRUP_MSG_JOIN_CMD::SRUP_MSG_JOIN_CMD()
{
    m_msgtype[0] = SRUP::SRUP_MESSAGE_TYPE_JOIN_CMD;
    m_device_id = nullptr;
}
...
bool SRUP_MSG_JOIN_CMD::DeSerialize(const unsigned char* serial_data)
{
    ...
    for (int i=0;i<8;i++)
    {
        std::memcpy(&dev_bytes[i], (uint8_t*) serial_data + p, 1);
        ++p;
    }
...
bool SRUP_MSG_JOIN_CMD::DataCheck()
{
    if ((m_token == nullptr) || (m_sequence_ID == nullptr) || (m_sender_ID == nullptr) || (m_device_id == nullptr))
        return false;
    else
        return true;
}

const uint64_t* SRUP_MSG_JOIN_CMD::device_ID()
{
    return m_device_id;
}
